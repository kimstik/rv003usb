...
def parse_dump(prefix):
    ...
    # Decode-path (post-quantisation, post-interpolation) params, dumped by
    # c2sim's synthesis loop (dump_lsp_/dump_ak_): the values actually used
    # by aks_to_M2 + synthesis on the 10 ms grid.  Present with --lpc.
    lsp_d = _load(prefix + "_lsp_.txt")
    if lsp_d is not None:
        out["lsp_dec"] = np.atleast_2d(lsp_d)[:F]
    ak_d = _load(prefix + "_ak_.txt")
    if ak_d is not None:
        out["ak_dec"] = np.atleast_2d(ak_d)[:F]

    # qmodel Wo/L (decoded pitch track — differs from _model.txt under
    # quantisation/decimation, e.g. c2sim --rate 1300)
    if os.path.exists(qpath):
        out["Wo_dec"] = qWo
        out["L_dec"] = qL
    return out
